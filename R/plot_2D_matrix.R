#' Plot bidimensional reduction of a matrix
#'
#' @importFrom igraph V
#' @import visNetwork
#' @import tidyverse
...
#' @param id_anno_color name of the column in \emph{nodes_anno} whose values are mapped to colors to observations
#' @param id_anno_shape name of the column in \emph{nodes_anno} whose values are mapped to shapes to observations
...
#' @param shapes shapes list to be used for plotting, if id_anno_shape is not NA
...
plot_2D_matrix <- function(coord, nodes_anno, id_name, id_anno_color = NA, id_anno_shape = NA,
  ...
  nodes_anno <- nodes_anno %>% dplyr::filter(nodes_anno[[id_name]] %in% base::rownames(coord))
  ...
  if (!is.na(id_anno_shape)) {
    if (is.null(shapes)) {
      ...
      vals <- vals[!(stringr::str_ends(vals, "down") | stringr::str_ends(vals, "up") |
                       ...
                       stringr::str_ends(vals, "open") | stringr::str_ends(vals, "ne") |
                       stringr::str_ends(vals, "se") | stringr::str_ends(vals, "sw") |
                       stringr::str_ends(vals, "nw"))]
    } else {
      vals <- shapes
    }
  }
  ...
  if (is.na(id_anno_color) & is.na(id_anno_shape)) {
    if (interactive) {
      gPlot <- gPlot_data %>% plotly::plot_ly(x = ~V1, y = ~V2, type = "scatter",
                                              ...
                                              colors = "Set2") %>%
        plotly::layout(xaxis = list(zeroline = FALSE), yaxis = list(zeroline = FALSE),
                       title = title)
    } else {
    ...
    if (interactive) {
      ...
      gPlot <- ggplot(gPlot_data, aes(x = V1, y = V2)) +
        geom_point(aes(shape = group2), color = "blue", size = size) +
        theme_bw() +
